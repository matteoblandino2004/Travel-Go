...
import { searchFlights, searchHotels } from '../src/data/provider.js';
...
test('the same query returns the same market twice', () => {
  const a = searchFlights({ from: 'SFO', to: 'TYO', date: '2026-10-12' });
  const b = searchFlights({ from: 'SFO', to: 'TYO', date: '2026-10-12' });
  assert.deepEqual(a.offers, b.offers);
});

test('unknown cities are rejected rather than silently returning nothing', () => {
  assert.throws(() => searchFlights({ from: 'SFO', to: 'Atlantis' }), /Unknown destination/);
  assert.throws(() => searchFlights({ from: 'SFO', to: 'SFO' }), /same city/);
  assert.throws(() => planTrip({ from: 'SFO', to: 'Atlantis', flight: neutralFlight, hotel: neutralHotel }), /don't have inventory/);
});

test('short-haul carriers are kept off intercontinental routes', () => {
  const { offers } = searchFlights({ from: 'SFO', to: 'TYO' });
  assert.ok(!offers.some((o) => o.airlineCode === 'FR'), 'Ryanair should not fly SFO-Tokyo');
});

test('caring only about price puts the cheapest flight first', () => {
  const { offers } = searchFlights({ from: 'SFO', to: 'TYO', date: '2026-10-12' });
...
test('caring only about layovers puts a nonstop first', () => {
  const { offers } = searchFlights({ from: 'SFO', to: 'TYO', date: '2026-10-12' });
...
test('a departure window pulls flights in that window up the list', () => {
  const { offers } = searchFlights({ from: 'SFO', to: 'TYO', date: '2026-10-12' });
...
test('planTrip returns both halves ranked, with reasons attached', () => {
  const trip = planTrip({
    from: 'San Francisco',
    to: 'Tokyo',
    date: '2026-10-12',
...
test('elite status shows up as lounge access and richer earning', () => {
  const plain = searchFlights({ from: 'SFO', to: 'TYO', date: '2026-10-12' });
  const elite = searchFlights({ from: 'SFO', to: 'TYO', date: '2026-10-12', profile: { alliances: { star: 'gold' } } });
